...
for device in os.listdir(root_folder):
    ...
    for modal in os.listdir(os.path.join(root_folder,device)):
        ...
        for user in os.listdir(os.path.join(os.path.join(root_folder,device), modal)):
            if user.startswith('.'):
                continue
            if user_data.get(modal) is None:
                user_data[modal] = {}
                user_labels[modal] = {}
            user_num = re.search(r'_(\d+)_', user)
            if user_num:
                if user_data[modal].get(user_num.group(1)) is None:
                    user_data[modal][user_num.group(1)] = []
                    user_labels[modal][user_num.group(1)] = [] #ACC AND GYRO NOT ALIGNED
            else:
                exit(code=1)
            ...
            data = data.to_numpy()
            user_data[modal][user_num.group(1)].append(data[:,-3:])
            user_labels[modal][user_num.group(1)].append(data[:,1])

for modal in user_data:
    for user in user_data[modal]:
        user_data[modal][user] = np.vstack(user_data[modal][user])
        user_labels[modal][user] = np.hstack(user_labels[modal][user])
...
segmented_data = {}
segmented_labels = {}
for modal in user_labels.keys(): # get the modality key
