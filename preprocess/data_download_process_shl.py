...
download_folder = "../datasets/download"
extracted_folder = "../datasets/extracted"
for i in range(len(fileName)):
    data_directory = os.path.abspath(download_folder+str(fileName[i]))
    if not os.path.exists(data_directory):
        print("downloading "+str(fileName[i]))
        download_url(links[i],data_directory)
        print("download done")
        data_directory2 =  os.path.abspath(extracted_folder +str(fileName[i]))
        print("extracting data...")
        with zipfile.ZipFile(data_directory, 'r') as zip_ref:
            zip_ref.extractall(os.path.abspath(extracted_folder))
...
rootDirectory = '../datasets/extracted/SHLDataset_preview_v1'
    ...
    for timeDir in timeSubFolder:
        ...
        for i in reversed(range(len(deleteRange))):
            ...
            for bodyCount in range(len(bodyLocations)):
                dataPosition[bodyCount] = np.delete(dataPosition[bodyCount], np.s_[deleteRange[i][0]:deleteRange[i][1]],
                                                    axis=0)
